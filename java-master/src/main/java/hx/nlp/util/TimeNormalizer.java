...
public class TimeNormalizer   implements Serializable {
	...
	private Pattern readModel(ObjectInputStream in) throws Exception {
		...
		String pp = p.pattern().replaceAll("月份", "月份?");
		System.out.println(p.pattern());

		Matcher matcher = Pattern.compile("3月份?").matcher("3月份");
		System.out.println(matcher.matches() ? matcher.group() : "N");
		...
		ObjectOutputStream oos = new ObjectOutputStream(
				new BufferedOutputStream(
						new GZIPOutputStream(
								new FileOutputStream("data/model/p.m"))));
		oos.writeObject(Pattern.compile(p.pattern() + "|abc"));

		return Pattern.compile(pp);
	}
