...
public class TemporalParserDemo {
	...
	public static void main(String[] args) {
		...
		String text = "大概在去年8月7号到今年9月的那段美好时光；昨天到今天上午八点左右;在去年的第二个季度；前年3月份的那段时光；" +
		...
		for (CoreMap sentence: annotation.get(CoreAnnotations.SentencesAnnotation.class)) {
			...
			List<TokenSequencePattern> patterns = Stream.of(
					...
					"[{tag:P}]? (?$date ([{tag:NT}] [{tag:DEG}]?){1,2}) ([{tag:/LC|AD/}])?",     // since until or during pattern

					"[{tag:P}]? (?$date [{tag:NT}]{1,2}) ([{tag:LC}])? [{tag:DEG}]? [{tag:DT}]? ([{tag:/CD|OD/}] [{tag:M}]? /分钟|刻钟|小时|天|日|星期|礼拜|月份?|季度?/ | [{tag:/M|JJ/}]? /时间|时候|时期|时光/) [{tag:LC}]?",         // A during B pattern
