...
public class TemporalParserDemo {
	...
	public static void main(String[] args) {
		...
		String text = "大概在去年8月7号到今年9月的那段美好时光里他一直在炒股；昨天到今天上午八点左右;" +
				"在上一年的第二个季度；前年3月份的那段时光；今年三月的第一个星期三；" +
				...
				"楼市在过去的5年里出现了大量的泡沫;去年上半年；昨天前几个小时；前几天早上；春秋时期；在前年股市大涨那段时间";
		...
		for (CoreMap sentence: annotation.get(CoreAnnotations.SentencesAnnotation.class)) {
			...
			List<TokenSequencePattern> patterns = Stream.of(
					...
					"([{tag:/LC|DT|NT|JJ/}])? ([{tag:/CD|OD/}] [{tag:M}]? /分钟|刻钟|小时|天|日|星期[一二三四五六日天]?|礼拜[一二三四五六日天]?|月份?|季度?|年/) ([{tag:/LC|JJ/}])? [{tag:DEG}]? ([{tag:NT}]{0,2})")
