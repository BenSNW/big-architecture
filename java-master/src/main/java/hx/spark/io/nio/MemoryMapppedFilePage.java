 ...
 * 
 * </p>Create a file, write some data to it, then create three different types
 * of mappings to it. </p>Observe the effects of changes through the buffer APIs
 * and updating the file directly. </p>The data spans page boundaries to
 * illustrate the page-oriented nature of Copy-On-Write mappings.
 *
 * @author BenSNW
 */
public class MemoryMapppedFilePage {
	
	public static void main(String[] argv) throws Exception {
		
		// create a temp file and get a channel connected to it
		File tempFile = File.createTempFile("mmap", null);
		...
		// This may cause a file hole, depending on the fs page size.
		buffer.clear();
		...
		// This modification is not visible to the Copy-On-Write buffer
		// since the memory page corresponding to the fs page has been modified by it
		rw.position(9);
		rw.put(" R/W ".getBytes());
		// This modification is visible to all the three buffer
		// since that file page is still shared by all the corresponding memory pages
		rw.position(8194);
		rw.put(" R/W ".getBytes());
		rw.force();
		...
		// Write to the file through the channel, hit all pages
		buffer.clear();
